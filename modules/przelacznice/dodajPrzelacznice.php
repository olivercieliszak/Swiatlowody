...
if(isset($_P -> punktID)){
	...
	$zapytanie1 = $FP -> db_iq('INSERT INTO punkt SET gps = "'.$zapytanie0 -> gps.'", opis = "Przełącznica w punkcie '.$zapytanie0 -> opis.'", punktTypID = "2"');

	$zapytanie = $FP -> db_iq('INSERT INTO przelacznica SET punktID = "'.$zapytanie1.'", opis = "'.$_P -> opis.'"');
	$test = false;
	if($zapytanie > 0)
		$test = true;

	echo $FP -> komunikat($test, 'Nowa przełącznica została dodana prawidłowo<br><br><a href="?modul=przelacznice&co=listaPrzelacznic">Powrót do listy przełącznic</a>',
	'Wystąpił błąd podczas dodawania przełącznicy<br><br><a href="?modul=przelacznice&co=listaPrzelacznic">Powrót do listy przełącznic</a>');
	if($test)
		$FP -> log('Dodano przełącznicę nr '.$zapytanie);
}
...
<form action="?modul=przelacznice&co=dodajPrzelacznice" method="POST">
<table align="center">
<tr>
<td colspan="2"><h3>Dodawanie nowej przełącznicy</h3></td>
</tr>
<tr>
<td>Punkt</td>
<td><?php
	...
	$zapytanie = $FP -> db_q('SELECT punktID, punktTypID, kolorPunkt, typ, opis, gps FROM punkt NATURAL LEFT JOIN punktTyp WHERE punkt.punktTypID != 2 ORDER BY punktID ASC');
